...
<template>
  <div class="p-4">
    ...
    <button @click="isOpen = !isOpen" class="px-4 py-2 bg-white text-text-base border border-gray-300 rounded-base text-sm font-medium shadow-sm hover:bg-gray-50 transition-colors duration-200 mb-4">
    ...
    <div v-if="isOpen" class="p-4 bg-gray-50 rounded-lg border border-gray-200 mb-6">
      <div class="mb-4">
        ...
        <input id="email" v-model="inputEmail" type="email" class="block w-full px-3 py-2 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary text-sm" />
      </div>
      <div class="mb-4">
        <label for="reason" class="block text-sm font-medium text-text-base mb-1">이름 / 사유 : </label>
        <input id="reason" v-model="inputReason" type="text" class="block w-full px-3 py-2 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary text-sm" />
      </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-subtle bg-white">
      <table class="w-full border-collapse min-w-full divide-y divide-gray-200">
        ...
        <tbody>
          <tr v-for="data in emailData" :key="data.id" class="divide-y divide-gray-100 hover:bg-gray-50">
            <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">{{ data.id }}</td>
            <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">{{ data.reason }}</td>
            <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">{{ data.email }}</td>
