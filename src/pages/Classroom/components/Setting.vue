...
// 수정 대상 세트
const handleSetPut = (data) => { // Renamed from classroomData to data to avoid conflict
  postData.value = {
    ...
    <table class="w-full border-collapse min-w-full divide-y divide-gray-200">
      <thead>
        <tr>
          ...
          <th class="bg-gray-50 text-text-muted font-medium text-sm py-2 px-3 border-b border-gray-200 text-center">제한</th>
          <th class="bg-gray-50 text-text-muted font-medium text-sm py-2 px-3 border-b border-gray-200 text-center">수정</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="data in classroomData" :key="data.grade_id" class="divide-y divide-gray-100 hover:bg-gray-50">
          <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">{{ data.grade_id }}학년</td>
          <!-- 제한 -->
          <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">
            <div v-if="postData.grade_id == data.grade_id">
              <input type="number" v-model="postData.required_count"
                     class="w-20 px-2 py-1 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary text-sm inline-block mr-1" />명 이상
            </div>
            <div v-else>
          ...
          <td class="text-sm py-2 px-3 border-b border-gray-100 text-center">
            ...
            <div v-else>
              <button @click="handleSetPut(data)" class="px-3 py-1 bg-white text-text-base border border-gray-300 rounded-base text-sm font-medium shadow-sm hover:bg-gray-50 transition-colors duration-200">수정</button>
            </div>
          </td>
        </tr>
      </tbody>
