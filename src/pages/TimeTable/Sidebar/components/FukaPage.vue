    ...
    <div class="flex flex-col gap-4">
      ...
      <div v-if="selectType == 'default'" class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-[120px_1fr] items-baseline gap-y-4">
          ...
          <select
            id="day"
            v-model="postFukaData.day"
            class="block w-full px-3 py-2 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
          >
            <option v-for="(day, idx) in days" :value="enDays[idx]" :key="idx">
              {{ day }}요일
            </option>
          </select>
        </div>
      ...
      <!-- Classroom -->
      <div class="grid grid-cols-[120px_1fr] items-baseline gap-y-4">
        <label class="block text-sm font-medium text-text-base pt-2" for="classroom">
          장소 입력:
        </label>
        <input
          id="classroom"
          v-model="postFukaData.room"
          class="block w-full px-3 py-2 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
        />
      </div>
      ...
      <div class="grid grid-cols-[120px_1fr] items-baseline gap-y-4">
        ...
        <div class="flex items-center gap-2">
          <select
            id="time-start"
            v-model="postFukaData.startTime"
            class="block w-full px-3 py-2 border border-gray-300 rounded-base shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
          >
    ...
    <div class="mt-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
      ...
      <ul class="list-none p-0 m-0 flex flex-col gap-2 mt-4">
        <li
          ...
          :key="user.user_id"
          class="flex justify-between items-center bg-white p-2 rounded-md text-sm text-text-base shadow-sm"
        >
          <span>- {{ user.name }} : {{ setTarget(user.grade_id) }}</span>
          <button
            @click="handleDelete(user.user_id)"
            class="px-3 py-1 bg-red-500 text-white text-xs rounded-sm hover:bg-red-600"
          >
            취소
          </button>
        </li>
...
const handleSubmit = async () => {
  ...
  if (
    postFukaData.value.section &&
    (postFukaData.value.date || postFukaData.value.day) &&
    postFukaData.value.room &&
    postFukaData.value.startTime &&
    postFukaData.value.endTime &&
    postFukaData.value.student_ids
  ) {
    // default 저장
