<template>
  <aside
    :class="{
      ...
      'transform -translate-x-full': !uiStore.isMobileSidebarOpen && !uiStore.isDesktop, // Mobile hidden
      'transform translate-x-0': uiStore.isMobileSidebarOpen, // Mobile open
      'w-64': uiStore.isSidebarWide, // Desktop wide
      'w-16': uiStore.isSidebarMinimized, // Desktop icon mode
    }"
  ...
  >
    <div class="h-14 flex items-center justify-center border-b border-gray-200 px-4">
      ...
      <router-link to="/" class="flex items-center text-lg font-semibold text-text-heading whitespace-nowrap">
        <span v-if="uiStore.isSidebarWide">GSC Portal</span>
        <span v-else>GP</span>
      </router-link>
    </div>
      ...
      <ul>
        <li v-for="item in navItems" :key="item.name" class="mt-1">
          ...
          >
            ...
            <span v-if="uiStore.isSidebarWide" class="ml-3">{{ item.name }}</span>
...
const handleResize = () => {
  ...
  const isDesktopCurrent = windowWidth >= 1024; // Tailwind's lg breakpoint

  uiStore.setIsDesktop(isDesktopCurrent); // Update isDesktop in store

  // If moving from mobile to desktop, ensure mobile sidebar is closed
  if (isDesktopCurrent && uiStore.isMobileSidebarOpen) {
    uiStore.setMobileSidebar(false);
  }

  // Adjust tablet sidebar minimized state based on window size
  if (windowWidth >= 768 && windowWidth < 1024) { // md breakpoint range
    uiStore.setTabletSidebarMinimized(true); // Always minimized on tablet
  } else if (windowWidth < 768) { // sm breakpoint range (mobile)
    // On mobile, the sidebar is a drawer, not minimized in the same way
    // Ensure it's not minimized for mobile, as it's a full drawer
    uiStore.setTabletSidebarMinimized(false);
  }
  // On desktop (lg breakpoint), we want to retain user preference for sidebar minimization.
  // The state uiStore.isTabletSidebarMinimized should only be changed by user action on desktop.
  // So no automatic change here for the desktop state.
};
